...
const SRC_ORDER = [
  'src/audio.js', 'src/input.js', 'src/rig.js', 'src/moves.js',
...
for (const banned of ['</script', 'import ', 'export ']) {
  ...
  if (banned !== '</script' && idx !== -1) {
    // report leftover module syntax at line starts only
    const bad = gameCode.split('\n').filter((l) => l.startsWith(banned.trim() + ' '));
    if (bad.length) throw new Error(`unstripped module syntax: ${bad[0]}`);
  }
}
